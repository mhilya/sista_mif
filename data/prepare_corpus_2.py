...
def process_jobstreet_kaggle(file_path: Path) -> pd.DataFrame:
    if not file_path.exists():
        print(f"  ⚠️ Jobstreet dataset not found: {file_path}")
        return pd.DataFrame()
    
    print("[3/4] Processing Jobstreet Kaggle Dataset (JOB TITLE ONLY)...")
    ...
    for chunk in pd.read_csv(str(file_path), encoding='utf-8-sig', dtype=str, chunksize=50000, on_bad_lines='skip'):
        ...
        if text_col not in chunk.columns: continue
            
        # Filter IT HANYA berdasarkan jobTitle
        it_pattern = '|'.join(IT_KEYWORDS)
        chunk_filtered = chunk[chunk[text_col].fillna('').str.contains(it_pattern, case=False, na=False, regex=True)].copy()
        
        if len(chunk_filtered) > 0:
            chunks.append(chunk_filtered)
    
    if not chunks:
        print("  ⚠️ No IT jobs found after filtering by title.")
        return pd.DataFrame()
    
    df = pd.concat(chunks, ignore_index=True)
    print(f"  Filtered to {len(df)} IT jobs by TITLE ONLY.")
    
    # Ekstrak job_text HANYA dari jobTitle
    ...
    print(f"Loaded {len(result)} IT job titles from Jobstreet")
    return result
...
def main():
    ...
    if not df_int.empty and "label_verified" in df_int.columns:
        ...
        test_output = OUTPUT_DIR / "test_set_internal_2.csv"
        ...
        print(f"\n📋 TEST SET TERSIMPAN: {len(df_test)} records → test_set_internal_2.csv")
    ...
    output_file = OUTPUT_DIR / "training_corpus_2.csv"
